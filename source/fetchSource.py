...
# This script is a portion of the configure.py located in the
# amazon-freertos/tools/configuration directory. It has been
# abstracted to allow users to disable/enable libraries before
# cloning the source code in the hopes that this will allow their
# downloads to be smaller by not downloading libararies that they don't need.
...
# This is used by both getBoardChoice function and the getVenderChoice
...
def getValidUserInput(options, max_acceptable_value, error_message):
    ...
    while 1:
        ...
        if(user_input.isdigit()):
            ...
            # Given the user entered a digit, make sure that the number
...
# getBoardChoice: Prints all of the possible boards options that the user can
# select from. It then calls getValidUserInput to ensure a valid selection.
...
# getVendorChoice: This function prints all of the possible vendor options that
# the user can select from. It then calls getValidUserInput to makes sure that
...
# setLibraryDefaults: Runs merge_config on the default configuration located in
# KConfig with the defaults for the board the user chose located at
#  '"../vendors/" + vendor_name + "/" + board + "/KConfig"'. This merged
# configuration is what is inevitably shown to the user in the GUI.
...
# enableLibraries: Calls guiconfig which is what takes the Kconfig file and
# presents the user with a GUI with the possible configuration options. When
# guiconfig is done running it outputs the users configuration decisions to
# .config
def enableLibraries():
    subprocess.run(["guiconfig"])


# getOutputDirectory: Asks the user to name the directory that the FreeRTOS
# source code will end up cloning in to.
...
# cloneFreeRTOSRepository: changes directories outside the scope of this
# scripts directory and clones the FreeRTOS source code.
